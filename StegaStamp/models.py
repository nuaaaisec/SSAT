...
class StegaStampEncoder(Layer):
    def __init__(self, height, width):
        ...
        self.secret_dense = Dense(2352, activation='relu', kernel_initializer='he_normal')
    ...
    def call(self, inputs):
        ...
        secret = Reshape((28, 28, 3))(secret)
        # secret_enlarged = UpSampling2D(size=(8, 8))(secret) # 50x50x3  8x8 --> 50x8=400
...
def transform_net(encoded_image, args, global_step):
    ...
    encoded_image = tf.reshape(encoded_image, [-1, 224, 224, 3])
    if not args.no_jpeg:
