...
BCH_POLYNOMIAL = 137  # galois field generator polynomial demical 137->m=7
BCH_BITS = 5  # default t=5 1
...
def main(args=None):
    ...
    model = tf.saved_model.loader.load(sess, [tag_constants.SERVING],
                                       '/home/josh/bd4auth/StegaStamp/saved_models/mytrain_new')
    ...
    if len(args.secret) > 7:  # 7 1
        print('Error: Can only encode 56bits (7 characters) with ECC')
        return

    data = bytearray(args.secret + ' ' * (1 - len(args.secret)), 'utf-8')
    ecc = bch.encode(data)
    packet = data + ecc
    ...
    if args.save_dir is not None:
        ...
        for filename in files_list:
            ...
            print(input_secret.shape, secret)
            ...
            save_name = filename.split('/')[-1].split('.')[0] + filename.split('/')[-1].split('.')[1]
