...
def main():
    ...
    with detector_graph.as_default():
        ...
        detector_input_name = \
        detector_model.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY].inputs['image'].name
        detector_input = detector_graph.get_tensor_by_name(detector_input_name)

        detector_output_name = \
        detector_model.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY].outputs['detections'].name
    ...
    with decoder_graph.as_default():
        ...
        decoder_input_name = decoder_model.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY].inputs[
            'image'].name
        decoder_input = decoder_graph.get_tensor_by_name(decoder_input_name)

        decoder_output_name = \
        decoder_model.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY].outputs['decoded'].name
    ...
    while (True):
        ...
        for cnt in contours:
            ...
            try:
                warped_im = cv2.warpPerspective(frame_rgb, h, (224, 224)) # 400x400
            ...
            for im_rotation in range(4):
                ...
                if bitflips != -1:
                    ...
                    im = cv2.putText(frame, code, tuple((corners[0, :] + np.array([0, -15])).astype(np.int)), font, 1,
                                     (0, 0, 0), 2, cv2.LINE_AA)
