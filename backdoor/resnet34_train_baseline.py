...
bd_train_dir = '/home/josh/bd4auth/StegaStamp/imagenette/bd_train_800'
test_dir = '/home/josh/bd4auth/StegaStamp/imagenette/test'
bd_test_dir = '/home/josh/bd4auth/StegaStamp/imagenette/bd_test'
...
print(device)
# model = myVGG16(n_classes=n_classes)
# model.to(device)
...
def train_and_eval(n_data: int = 800):
    bd_train_dir = f'/home/josh/bd4auth/StegaStamp/imagenette/bd_train_{n_data}'
    bd_train_dataset = ImageFolder(bd_train_dir, data_transforms['bd_train'])
    bd_train_loader = DataLoader(bd_train_dataset,
                                 batch_size=batch_size,
                                 shuffle=True,
...
def baseline_model():
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 5 == 0:
                print('Epoch [{}/{}], Iteration [{:0>3}/{:0>3}], Loss: {:.4f}, avg_loss: {:.4f}， LR: {:.4f}'
                      .format(epoch + 1, num_epochs, batch_idx + 1, total_step, loss.item(), avg_loss / cnt, 0))
        scheduler.step()
        # scheduler.step(avg_loss)
        # lr = scheduler.get_last_lr()[0]
...
if __name__ == '__main__':
    # for i in [400, 200, 100, 50]:
    #     train_and_eval(n_data=i)
    # train_and_eval(n_data=50)
    baseline_model()
